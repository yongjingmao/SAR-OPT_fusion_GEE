...
def prepare_optical(optical_collection, AOI, optical_mission):
    ...
    def add_cloudMask_S2(img):
        ...
        Number 3 is cloud shadow and above 7 are cloud with different confidence respectively.
        In the cloud mask, 1 is for cloudy pixel.
        """
...
def cal_covariates(img, AOI, indep_variables):
    ...
    """Step 4: Spatially smooth the input independant bands by applying a median smoother to each band
...
def post_process(paired_collection, prediction, 
    ...
    def pred_standardize(img):
        ...
        standardizing prediction according to the observation at the same time"""
    
        # Get the id of opt-SAR pair
        # Find the corresponding smoothed image from the multibands NDVI_smoothed
        ...
        # clear pixels
        reducer = ee.Reducer.mean().combine(ee.Reducer.intervalMean(85, 95), '90', True)\
            .combine(ee.Reducer.intervalMean(5, 15), '10', True)
        ...
        NDVI_calibrated = NDVI_upcalibrated.where(NDVI_diff.lt(0), NDVI_downcalibrated)
        NDVI_calibrated = ee.Algorithms.If(
            cloud_cover.lt(std_cloud_threshold), 
            NDVI_calibrated, 
            NDVI_pred)
        return img.addBands(ee.Image(NDVI_calibrated).rename('NDVI_pred'), overwrite=True)
