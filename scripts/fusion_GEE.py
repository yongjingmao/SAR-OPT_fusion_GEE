...
def main():
    ...
    # Name of Input optical missions, choose from L8 (Landsat8) and S2 (Sentinel 2)
    ...
    # The ratio used to split training and testing (1 means all data for training)
    TRAIN_TEST_SPLIT = cfg["TRAIN_TEST_SPLIT"]
    # Ration of valdation dataset which is split from training dataset with artificial cloud mask
    ...
    if EXPORT_INPUT:
        ...
        collection_time = optical_collection.aggregate_array('system:time_start')
    ...
    if len(PATH_MATADATA)==0:
        ...
        Random_dict = ee.Dictionary.fromLists(list(metadata['ID']), list(metadata['Random']))
        SplitLabel_dict = ee.Dictionary.fromLists(list(metadata['ID']), list(metadata['SplitLabel']))
        Cloud_dict = ee.Dictionary.fromLists(list(metadata['ID']), list(metadata['Cloud_cover']))
    ...
    # Convert imagecollection to multiband images
    OutputFilled = NDVI_filled.select('NDVI').toBands().clip(AOI)  # Output NDVIs
    OutputMask = NDVI_filled.select('Mask').toBands().clip(AOI)  # Output Cloud Mask
